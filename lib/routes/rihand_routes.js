    ...
    var internalRoutingClosure  = (request, response, next)  => {
      ...
      var renderResponse = () => {
        if(controller.format == 'json') {
          ...
          response.render(`${controllerName.toLowerCase()}/${effectiveActionName}.handlebars`,viewData);
        }
        else {
...
class RihandRoutes {
  ...
  namespace (key)  {
    ...
    if(this.namespaces[key] === undefined || this.namespaces[key] === null){
      this.namespaces[key] = new RihandRoutesNamespace(this.options);
    }
...
RihandRoutes.instance = null;

RihandRoutes.getInstance = function(opts){
  if(RihandRoutes.instance === null){
    RihandRoutes.instance = new RihandRoutes(opts);
  }
  return RihandRoutes.instance;
};
