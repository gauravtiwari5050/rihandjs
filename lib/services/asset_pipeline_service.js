...
class AssetPipelineService extends require('./service').Service {
  connectAssetsConfiguration() {
    ...
    configuration.paths = _.map(configuration.paths, function (filePath) { //eslint-disable-lint
  ...
  initialize(options, callback) {
    this.connectAssetsInstance = connectAssets(this.connectAssetsConfiguration());
    callback(null);
  }
