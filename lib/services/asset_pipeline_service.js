...
class AssetPipelineService extends require('./service').Service {
  connectAssetsConfiguration() {
    ...
    // disable uglify by default
    configuration.compress = this.application.configuration.value('compressAssets', false);
    ...
    configuration.buildDir = '/' + path.resolve(self.application.root(), `public/${assetPath}`);
    console.log(configuration.buildDir);
