...
var Routes = require('../routing/routes_v2').Routes;
...
class WebService extends require('./service').Service {
  ...
  setupViewEngine () {
    ...
    connectAssetsConfiguration.compile = this.app.applicationConfig.value('compile_assets', false);
    
    // write assets to disk in production environment
    connectAssetsConfiguration.build = this.app.applicationConfig.value('write_assets_to_disk', true);

    // set fingerprint to true by default
    connectAssetsConfiguration.fingerprinting = this.app.applicationConfig.value('fingerprint_assets', true);

    var asset_host = this.app.applicationConfig.value('asset_host');

    if(asset_host){
      connectAssetsConfiguration.servePath = `${asset_host}/assets`;
    }

    connectAssetsConfiguration.buildDir = 'public/assets';
  ...
  start(command_line_args,callback){
    var self = this;
    ...
    var port = command_line_args.getValue('port');
    if(_.isNil(port)){
      port = '5050';
    }
    port = parseInt(port);
