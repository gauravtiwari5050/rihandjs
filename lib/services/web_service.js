...
class WebService extends require('./service').Service {
  ...
  async setupHttpsServer() {
    ...
    const sniPath = `${this.application.root()}/config/ssl/sni/sni.js`;
    if ((await File.exists(sniPath))) {
      const SNI = require(sniPath);
      if (SNI && _.isFunction(SNI.certificate)) {
        sslOptions.SNICallback = function (domain, cb) {
          SNI.certificate(domain).then((options) => {
