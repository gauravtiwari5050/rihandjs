...
class Controller {
  ...
  getMimeObject(format) { // eslint-disable-line
  ...
  renderResponse(request, response, next) {
    ...
    const format = this.format || 'html';
    const mimeObject = this.getMimeObject(format);
    ...
    if (mimeObject.template === true) {
      renderedObject = this.getViewVariables();
    } else {
      renderedObject = JSON.stringify(this.getJson());
    }
