...
class Configuration {
  ...
  getYamlConfig(path, callback) {
    const self = this;
    const configPath = `${this.application.root()}/config/${path}`;
    if (_.isNil(callback)) {
      const yamlObject = YAML.load(configPath);
      return yamlObject[this.environment()];
    }
    return YAML.load(configPath, (yamlObject) => {
      if (_.isNil(yamlObject)) {
        return callback(`Invalid yaml at ${configPath}`);
      }
      return callback(null, yamlObject[self.environment()]);
    });
  }
